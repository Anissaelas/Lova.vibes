...
export default function LocaVibesApp() {
  ...
  return (
    <div className="min-h-screen bg-[#FFF0F5] font-sans text-gray-800 selection:bg-pink-300">
      ...
      {/* Views */}
      {currentView === 'home' && <HomeFeed onSelectSpot={navigateToSpot} allSavedSpotIds={allSavedSpotIds} onSaveClick={setSaveModalSpot} />}
      {currentView === 'all_cities' && <AllCitiesView onSelectCity={navigateToCityDetail} />}
      {currentView === 'city_detail' && <CityDetailView city={activeCityObj} onSelectSpot={navigateToSpot} onBack={() => setCurrentView('all_cities')} allSavedSpotIds={allSavedSpotIds} onSaveClick={setSaveModalSpot} />}
      ...
      {currentView === 'admin' && <AdminDashboard onBack={() => setCurrentView('profile')} />}
      {currentView === 'saved' && <SavedView lists={savedLists} allSpots={MOCK_SPOTS} onSelectSpot={navigateToSpot} allSavedSpotIds={allSavedSpotIds} onSaveClick={setSaveModalSpot} />}
      ...
      <header className="pt-12 px-5 pb-4 sticky top-0 z-40 bg-gradient-to-b from-[#FFD1DC]/90 via-[#FFD1DC]/80 to-transparent backdrop-blur-sm">
        ...
        {showFilters && (
          <div className="mt-4 p-4 bg-white/60 backdrop-blur-xl border border-white rounded-2xl shadow-lg animate-in slide-in-from-top-2">
            <h3 className="text-xs font-bold uppercase tracking-wider text-gray-400 mb-3">Vibe Filters</h3>
            <div className="flex flex-wrap gap-2">
              <span className="px-3 py-1 bg-pink-100 text-pink-700 rounded-lg text-xs font-medium border border-pink-200 cursor-pointer">🌸 Pink/Floral</span>
              <span className="px-3 py-1 bg-gray-100 text-gray-700 rounded-lg text-xs font-medium border border-gray-200 cursor-pointer">🍸 Date Night</span>
              <span className="px-3 py-1 bg-yellow-100 text-yellow-700 rounded-lg text-xs font-medium border border-yellow-200 cursor-pointer">📸 Best View</span>
            </div>
          </div>
        )}
      </header>
        ...
        <div className="px-5 mt-2 space-y-4">
          ...
          {spotsInList.length === 0 && (
            <div className="text-center py-20 bg-white/40 rounded-3xl border border-white border-dashed">
              ...
              <h3 className="text-lg font-bold text-gray-900">This list is empty</h3>
              <p className="text-sm text-gray-500 mt-2">Add spots using the heart icon.</p>
            </div>
          )}
        </div>
      </div>
      ...
      <header className="pt-16 px-5 pb-8 bg-white/60 backdrop-blur-md rounded-b-[3rem] shadow-sm text-center">
        ...
        <div className="flex justify-center gap-8 mt-6">
          ...
          <div className="text-center">
            ...
            <p className="text-xs text-gray-500 uppercase tracking-wider font-semibold">Lists</p>
          </div>
          <div className="text-center">
            <p className="text-2xl font-black text-gray-900">89</p>
            <p className="text-xs text-gray-500 uppercase tracking-wider font-semibold">Upvotes</p>
          </div>
        </div>
      </header>
      ...
      <div className="px-5 mt-8 space-y-3">
        <button className="w-full flex items-center justify-between p-4 bg-white rounded-2xl shadow-sm hover:shadow-md transition-all">
          <div className="flex items-center gap-3">
            <div className="p-2 bg-pink-50 text-pink-500 rounded-xl"><User className="w-5 h-5" /></div>
        ...
        <button className="w-full flex items-center justify-between p-4 bg-white rounded-2xl shadow-sm hover:shadow-md transition-all">
          <div className="flex items-center gap-3">
            <div className="p-2 bg-blue-50 text-blue-500 rounded-xl"><Settings className="w-5 h-5" /></div>
...
function SpotCard({ spot, isSponsored, rank, onClick, isSaved, onSaveClick }) {
  ...
  return (
    ...
    >
      ...
      <button 
        onClick={(e) => { e.stopPropagation(); onSaveClick(spot); }}
        className={`absolute top-4 right-4 z-10 p-2 backdrop-blur-md rounded-full transition-colors ${
          isSaved ? 'bg-pink-100 text-pink-500' : 'bg-white/30 text-white hover:bg-white/50 hover:text-pink-500'
        }`}
      >
      ...
      <div className="p-5">
        ...
        <div className="flex flex-wrap gap-2 mt-4">
          ...
          {spot.tags.map(tag => (
            <span key={tag} className="px-2 py-1 bg-white/50 border border-gray-100 text-gray-600 rounded-lg text-[10px] font-medium shadow-sm">
              {tag}
            </span>
          ))}
        </div>
...
function SpotDetail({ spot, onBack, onHaveBeenClick, isSaved, onSaveClick }) {
  ...
  const [draftRating, setDraftRating] = useState({ food: 0, service: 0, vibe: 0 });
  const [hasRated, setHasRated] = useState(false);
  
  const hasVoted = draftRating.food > 0 && draftRating.service > 0 && draftRating.vibe > 0;
  const draftOverall = hasVoted ? ((draftRating.food + draftRating.service + draftRating.vibe) / 3).toFixed(1) : 0;
    ...
    <div className="min-h-screen bg-[#FFF9E3] pb-32 animate-in slide-in-from-right-8 duration-300">
      <div className="relative h-80 w-full">
        ...
        <button 
          ...
          className={`absolute top-12 right-5 p-3 backdrop-blur-md rounded-full transition-colors ${
            isSaved ? 'bg-pink-100 text-pink-500' : 'bg-white/20 text-white hover:bg-white/40'
          }`}
        >
      ...
      <div className="px-5 -mt-4 relative z-10">
        <div className="bg-white/80 backdrop-blur-xl border border-white rounded-2xl p-4 shadow-xl flex justify-around mb-6">
          <a href="#" className="flex flex-col items-center gap-1 text-blue-600 hover:text-blue-500">
            <MapPin className="w-5 h-5" />
            <span className="text-[10px] font-bold uppercase tracking-wider">Address</span>
          </a>
          <a href="#" className="flex flex-col items-center gap-1 text-gray-700 hover:text-gray-500">
        ...
        <div className="flex flex-wrap gap-2 mb-8">
          ...
          {spot.tags.map(tag => (
            <span key={tag} className="px-3 py-1.5 bg-[#FFD1DC]/30 border border-[#FFD1DC] text-pink-800 rounded-xl text-xs font-medium">
              {tag}
            </span>
          ))}
        </div>
        ...
        <div className="bg-white/60 backdrop-blur-xl border border-white rounded-3xl p-6 shadow-lg mb-8">
          ...
          {hasRated ? (
            <div className="text-center py-6">
              ...
              <p className="text-sm text-gray-500 mb-4">You're helping the community.</p>
              <button onClick={() => setHasRated(false)} className="text-pink-600 text-sm font-semibold">Edit rating</button>
            </div>
          ) : (
            <div className="space-y-2">
              <DiamondRatingRow label="Food" icon={<Utensils className="w-4 h-4"/>} value={draftRating.food} onChange={(v) => setDraftRating({...draftRating, food: v})} />
              <DiamondRatingRow label="Service" icon={<Heart className="w-4 h-4"/>} value={draftRating.service} onChange={(v) => setDraftRating({...draftRating, service: v})} />
              <DiamondRatingRow label="Vibe" icon={<Camera className="w-4 h-4"/>} value={draftRating.vibe} onChange={(v) => setDraftRating({...draftRating, vibe: v})} />
              ...
              <div className="pt-4 border-t border-gray-100 flex justify-between items-center mt-6">
                ...
                <div className="flex items-center gap-2">
                  <DiamondDisplay score={parseFloat(draftOverall)} size="w-5 h-5" />
                </div>
              </div>
              <button onClick={() => setHasRated(true)} disabled={!hasVoted} className={`w-full font-bold py-3 rounded-xl shadow-md mt-6 transition-all ${hasVoted ? 'bg-gradient-to-r from-pink-400 to-orange-400 text-white' : 'bg-gray-200 text-gray-400'}`}>
                Submit Vibe Check
              </button>
            </div>
          )}
        ...
        <div>
          ...
          <div className="flex gap-2 p-1 bg-white/50 backdrop-blur-md rounded-2xl mb-4 border border-white shadow-sm">
            <TabButton active={activeTab === 'view'} onClick={() => setActiveTab('view')} icon={<Camera className="w-4 h-4" />}>The View</TabButton>
            <TabButton active={activeTab === 'table'} onClick={() => setActiveTab('table')} icon={<Armchair className="w-4 h-4" />}>Best Table</TabButton>
            <TabButton active={activeTab === 'food'} onClick={() => setActiveTab('food')} icon={<Utensils className="w-4 h-4" />}>Food</TabButton>
          </div>

          <div className="space-y-4">
            {spot.galleries[activeTab]?.length > 0 ? (
               spot.galleries[activeTab].map(photo => (
                 <div key={photo.id} className="bg-white rounded-2xl p-2 shadow-sm border border-gray-100">
                   <div className="relative h-48 w-full mb-3 rounded-xl overflow-hidden group">
                     <img src={photo.url} alt="User submission" className="w-full h-full object-cover" />
                     {photo.time && <span className="absolute bottom-2 right-2 bg-black/60 backdrop-blur-md text-white text-[10px] px-2 py-1 rounded-md">{photo.time}</span>}
                     <span className="absolute top-2 left-2 bg-blue-500/80 backdrop-blur-md text-white text-[10px] px-2 py-1 rounded-md font-bold flex items-center gap-1"><CheckCircle className="w-3 h-3"/> Verified Location</span>
                   </div>
                   <div className="px-2 pb-1 flex justify-between items-start">
                     <div>
                       <p className="text-xs font-bold text-gray-900">@{photo.author}</p>
                       {photo.tip && <p className="text-sm text-gray-600 mt-1 italic">"{photo.tip}"</p>}
                     </div>
              ...
              <div className="text-center py-10 bg-white/40 rounded-2xl border border-white border-dashed">
                <p className="text-gray-500 text-sm">No photos here yet. Be the first!</p>
                <button className="mt-3 text-pink-500 font-semibold text-sm">+ Upload Photo</button>
              </div>
            )}
          </div>
        </div>
...
function HaveBeenView({ spot, onBack, onSubmit }) {
  ...
  const restaurantOptions = [
    ...
    'Vega/Vegan friendly', 'Gluten-free', 'Halal', 'Kosher', 'Great cocktails/Mocktails', 'Fine dining', 'Affordable luxury',
    'Instagrammable', 'Worth the hype', 'Worth the queue', 'Unique presentation', 'Food show', 'Hidden gem', 'Secret entrance',
    'Sunset view', 'Golden hour spot', 'Aesthetic interior', 'Dresscode required', 'Card only', 'Cash only', 'Hard to book'
  ];
  ...
  const beachclubOptions = [
    ...
    'Vega/Vegan friendly', 'Gluten-free', 'Halal', 'Kosher', 'Great cocktails/Mocktails', 'Instagrammable', 'Worth the hype',
    'Worth the queue', 'Unique presentation', 'Show', 'Hidden gem', 'DJ'
  ];

  const hotelOptions = [
    'View from bed', 'Outdoor bathtub / Jacuzzi', 'Private pool', 'Aesthetic bathroom', 'Boutique hotel', 'Adults only',
    'All-inclusive luxury', 'Rooftop pool', 'Rooftop Bar', 'Instagrammable lobby', 'Spa & Wellness', 'Day pass available',
    'Workation friendly', 'Luxury', 'Solo-friendly', 'Group-friendly', 'Anniversary/Romantic'
  ];
    ...
    <div className="min-h-screen bg-[#FFF0F5] pb-32 animate-in slide-in-from-bottom-8 duration-300">
      ...
      <div className="px-5 mt-6">
        <div className="bg-white rounded-3xl p-6 shadow-sm border border-white">
          ...
          <h2 className="text-center font-bold text-xl mb-1 text-gray-900">{spot.name}</h2>
          <p className="text-center text-sm text-gray-500 mb-8">What applied during your visit?</p>
...
function SaveModal({ spot, lists, onClose, onToggleInList, onCreateList }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-end justify-center bg-black/40 backdrop-blur-sm animate-in fade-in" onClick={onClose}>
      <div className="bg-white w-full max-w-md rounded-t-3xl p-6 pb-safe animate-in slide-in-from-bottom-2" onClick={e => e.stopPropagation()}>
            ...
            return (
              <button 
                ...
                className={`w-full flex items-center justify-between p-4 rounded-2xl border transition-colors ${
                  isSaved ? 'border-pink-500 bg-pink-50' : 'border-gray-200 bg-white hover:bg-gray-50'
                }`}
              >
                <div className="flex items-center gap-3">
                  <div className={`p-2 rounded-xl ${isSaved ? 'bg-pink-100 text-pink-500' : 'bg-gray-100 text-gray-500'}`}>
                    <Folder className="w-5 h-5" />
                  </div>
                  <span className={`font-bold ${isSaved ? 'text-pink-700' : 'text-gray-800'}`}>{list.name}</span>
                </div>
                <div className={`w-6 h-6 rounded-full flex items-center justify-center border-2 ${isSaved ? 'bg-pink-500 border-pink-500 text-white' : 'border-gray-300'}`}>
        ...
        <div className="flex gap-2">
          ...
          />
          <button 
            onClick={() => { if(newListName.trim()) { onCreateList(newListName.trim(), spot.id); setNewListName(''); } }} 
...
function DiamondRatingRow({ label, icon, value, onChange }) {
  return (
    <div className="flex justify-between items-center py-2">
      <span className="text-sm font-semibold flex items-center gap-2 text-gray-700">{icon} {label}</span>
      <div className="flex gap-1.5">
        {[1, 2, 3, 4, 5].map((idx) => (
          ...
          >
            <Gem 
              className={`w-5 h-5 ${idx <= value ? 'fill-pink-500 text-pink-500' : 'text-gray-200'}`} 
            />
          </button>
        ))}
      </div>
...
function DiamondDisplay({ score, size = "w-3 h-3" }) {
  ...
  return (
    <div className="flex gap-0.5" title={score.toFixed(1)}>
      {[1, 2, 3, 4, 5].map((idx) => {
        if (idx <= totalFull) return <Gem key={idx} className={`${size} fill-pink-500 text-pink-500`} />;
        if (idx === totalFull + 1 && hasHalf) return <Gem key={idx} className={`${size} fill-pink-300 text-pink-300`} />;
  ...
  return (
    <button 
      ...
      className={`flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl text-xs font-bold transition-all ${
        active ? 'bg-white text-pink-600 shadow-sm' : 'text-gray-500 hover:bg-white/50'
      }`}
    >
      {icon} {children}
    </button>
...
function AdminDashboard({ onBack }) {
  return (
    <div className="min-h-screen bg-gray-50 pb-32 animate-in fade-in">
      ...
      <div className="p-5">
        <h2 className="text-sm font-bold uppercase tracking-wider text-gray-500 mb-4">Pending Submissions</h2>
        
        <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-200 mb-4">
          <div className="flex justify-between items-start mb-3">
            <div>
              <span className="text-xs font-bold text-orange-500 bg-orange-100 px-2 py-1 rounded-md mb-2 inline-block">New City Request</span>
              <h3 className="font-bold">Bali, Indonesia</h3>
              <p className="text-xs text-gray-500">Submitted by @TravelGuru</p>
            </div>
          </div>
          <div className="flex gap-2 mt-4">
            <button className="flex-1 bg-green-500 text-white font-bold py-2 rounded-xl text-sm hover:bg-green-600">Publish</button>
            <button className="flex-1 bg-gray-100 text-gray-600 font-bold py-2 rounded-xl text-sm hover:bg-gray-200">Reject</button>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-200">
          <div className="flex justify-between items-start mb-3">
            <div>
              <span className="text-xs font-bold text-blue-500 bg-blue-100 px-2 py-1 rounded-md mb-2 inline-block">Photo Upload</span>
          ...
          <div className="h-32 bg-gray-100 rounded-xl overflow-hidden mb-3">
             <img src="https://images.unsplash.com/photo-1582650570392-809ab43f0be7?q=80&w=500&auto=format&fit=crop" className="w-full h-full object-cover" alt="pending" />
          </div>
          <div className="flex gap-2">
            <button className="flex-1 bg-green-500 text-white font-bold py-2 rounded-xl text-sm hover:bg-green-600">Approve</button>
            <button className="flex-1 bg-gray-100 text-gray-600 font-bold py-2 rounded-xl text-sm hover:bg-gray-200">Reject</button>
          </div>
        </div>
      </div>
...
function CityDetailView({ city, onSelectSpot, onBack, allSavedSpotIds, onSaveClick }) {
    ...
    <div className="min-h-screen bg-[#FFF0F5] pb-24 animate-in slide-in-from-right-4">
      ...
      <div className="px-5 mt-2 space-y-3">
            ...
            >
              ...
              <button 
                ...
                className={`p-2 rounded-full transition-colors ${
                  isSaved ? 'text-pink-500 bg-pink-50' : 'text-gray-300 hover:text-pink-500'
                }`}
              >
                <Heart className={`w-5 h-5 ${isSaved ? 'fill-current' : ''}`} />
              </button>
        ...
        {filteredSpots.length === 0 && (
          <div className="text-center py-10 text-gray-500">No spots found for this filter in {city?.name}.</div>
        )}
      </div>
    </div>
  );

...
import { Compass, LayoutGrid, Heart, User, MapPin, ChevronLeft, Search, Bookmark, Globe, Instagram, CalendarDays, Flame, LogOut } from 'lucide-react';
...
function SpotCard({ spot, onClick }) {
  return (
    <div onClick={() => onClick(spot)} className="min-w-[140px] bg-white p-2 rounded-2xl shadow-sm border border-gray-100 cursor-pointer">
      <div className="h-32 bg-gray-200 rounded-lg mb-2 overflow-hidden"><img src={spot.image || 'https://via.placeholder.com/150'} className="w-full h-full object-cover"/></div>
    ...
    <div className="p-5 animate-in slide-in-from-right duration-300">
      ...
      <div className="bg-white rounded-3xl p-6 shadow-xl border border-gray-100">
        ...
        <div className="flex gap-3 mb-6">
            {spot.websiteUrl && <a href={spot.websiteUrl} target="_blank" className="flex-1 bg-gray-100 p-3 rounded-xl text-center font-bold text-sm">Website</a>}
            {spot.instagramUrl && <a href={spot.instagramUrl} target="_blank" className="flex-1 bg-gray-100 p-3 rounded-xl text-center font-bold text-sm">Instagram</a>}
        </div>
